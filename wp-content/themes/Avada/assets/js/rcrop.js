...
function Draggable(el, options){
    ...
    var setScrollable = function(){
        ...
        scrollable.each(function(){
            ...
            if(!jQuerythis.length)
                return true;
            var overflow = isOverflowed(jQuerythis);
            if(!overflow.x && !overflow.y)
                return true;
            
            var o = jQuerythis[0].getBoundingClientRect();
            var dimension = getDimension(jQuerythis);
            var borderTop = parseInt(jQuerythis.css('border-top-width'));
            var borderLeft = parseInt(jQuerythis.css('border-left-width'));
            ...
            var obj = {
                ...
                isParent : self.settings.overflow ? false : self.el.offsetParent().is(jQuerythis),
                isBody : jQuerythis.is('body')
            };
            if(obj.isBody){
                var jQueryw = jQuery(window);
                obj.top = 0;
                obj.left = 0;
                obj.bottom = jQueryw.height();
                obj.right = jQueryw.width();
                obj.innerHeight = jQueryw.height();
                obj.innerWidth = jQueryw.width();
            }
    ...
    var isOverflowed = function(scrollable){
        ...
        if(scrollable.is('body')){
            if( jQueryb.height() > jQueryw.height())
                y = true;
            if( jQueryb.width() > jQueryw.width())
                x = true;
        }else{
    ...
    this.addDroppable = function(elements){
        ...
        droppables.each(function () {
            var jQuerythis = jQuery(this),
                offset = jQuerythis.offset(),
                height = jQuerythis.outerHeight(),
                width = jQuerythis.outerWidth(),
                borderTop =  parseInt(jQuerythis.css('border-top-width')),
                borderLeft = parseInt(jQuerythis.css('border-left-width')),
                inner = jQuery.clayfy.getInner(jQuerythis);
    ...
    var init = function(){
        ...
        self.draggableBox.on('clayfy-dragstart', function(){
            ...
            helper.each(function(){
                var jQuerythis = jQuery(this);
                if(jQuerythis.parent().is(self.droppableParent)){
                    self.draggable.addDroppable(jQuerythis);
                }
