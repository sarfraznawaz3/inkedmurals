...
Plugin URI: http://www.blueliquiddesigns.com.au
Description: Uploads a file to your Dropbox folder
Version: 1.0.8.1
Author: Blue Liquid Designs
Contributors: usableweb (http://wordpress.org/support/profile/usableweb)
...
class GFdropbox {
	...
	static public function init() {
		...
		add_action("gform_post_submission", array('GFdropbox', 'set_post_content'), 10, 2);
	}
     ...
     * 
     */	
	public static function set_post_content($entry, $form) {
		...
		foreach($form['fields'] as $fields)
		{
			...
			$dropbox_exists = ($fields['dropboxField'] == 1) ? true : false; 		
			...
			{				
				...
				if(!$id || strlen($entry[$id]) === 0)
				{
				  ...
				  */
				  $upload_path = (isset($fields['dropboxFieldPath'])) ? $fields['dropboxFieldPath'] : get_option("gf_dropbox_directory");
				
				 
				 /*
				  * Get details about the upload path
				  */
				 $extension = pathinfo($entry[$id], PATHINFO_EXTENSION);		
				 $file_url = pathinfo($entry[$id], PATHINFO_DIRNAME) . '/';		
				 $file_name = basename($entry[$id]);
								
				 				
				/* remove non-english characters */
				$file_name = preg_replace('/[^\00-\255]+/u', '', $file_name);												
				 ...
				 * Replace the macros
				 */				 
				 $override_path = self::replace_macros($upload_path, $entry['id']);	

				/*
				 * Turn the URL into the absolute path
				 */
				$dropbox_upload[] = array('dir' => $override_path, 'file_name' => $file_name, 'local_path' => str_replace(site_url().'/', ABSPATH, $entry[$id]));							
			}
		}				
		
		/* get the required keys and tokens */
		$app_key = get_option("gf_dropbox_key");
		$app_secret = get_option("gf_dropbox_secret");

		/* check if a user forgot to put a forwardslash in the path */
		...
		foreach($dropbox_upload as $file)
		{
			...
			$response = self::$dropbox->putFile($file['dir'].$file['file_name'], $file['local_path']);
