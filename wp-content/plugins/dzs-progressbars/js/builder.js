...
jQuery(document).ready(function($){
    ...
    var tinymce_settings = {
        ...
        ,mode : "textareas"
        ,theme : "modern"
        ,plugins : "image,media,hr,fullscreen"+includemediasupport
        ,relative_urls : false
...
//            console.info(_t.find('.with-tinymce.activated'))
            ...
            _t.find('.with-tinymce.activated').each(function(){
                var _t2 = $(this);

                _t2.removeClass('activated');


                // console.info(_t2);
                var ed_id = _t2.attr('id');
...
//            $( "#amount" ).val( ui.value );
    ...
    $(document).on('click','.delete-skin-btn', handle_mouse);
    ...
    _c.each(function(){
        ...
        if(_t.hasClass('activated')==false){
            ...
            _t.addClass('activated');
            window.tinyMCE.init(tinymce_settings);
        }
        setTimeout(function(){
            ...
            if(_t.hasClass('delete-skin-btn')){
                ...
                if(r){
                    ...
                    var data = {
                        ...
                        ,postdata: _t.attr('data-lab')
                        ,nonce: _t.attr('data-nonce')
                    };
                    $('.saveconfirmer').html('Options saved.');
                    $('.saveconfirmer').fadeIn('fast').delay(2000).fadeOut('fast');
                    $.post(ajaxurl, data, function(response) {
                        if(window.console !=undefined ){
                            console.log('Got this from the server: ' + response);
                        }
                        $('#save-ajax-loading').css('visibility', 'hidden');
                        _t.parent().slideUp("fast");
                    });
                }
...
//        return false;
        $.post(ajaxurl, data, function(response) {
            ...
            if(response.indexOf('success - ')>-1){
                ...
                $('.saveconfirmer').html('The configuration saved is the same or there is an error ... ');
            }
            setTimeout(function(){
...
//        console.info(_areaCanvas);
        ...
        if(_t.hasClass('builder-add-text')){
            ...
            if(_c.hasClass('activated')==false){
                ...
                _areaLayers.children().last().find('textarea.with-tinymce').addClass('activated');
...
//                        console.info(_t2, _t2.tinymce());
                        ...
                        if(_t2.hasClass('activated')){

                            console.info(_t2);

                            var el = (tinyMCE.get(_t2.attr('id')));



                            if(el){
                                console.info(el);
                                console.info(el.getContent());

                                var aux = '';

                                try{
                                    aux = (el.getContent({format: 'raw'}));
//                            console.info(aux);
