...
//            echo 'maxlen'.$maxlen;
        ...
        static function generate_input_text($argname, $otherargs = array()) {
            ...
            if ($margs['type'] == 'colorpicker') {
                $margs['class'].=' with_colorpicker from-dzs-functions';
            }
...
$(".with_colorpicker").each(function(){
        ...
        _t.bind("change", function(){
            //console.log(_t);
            $("#customstyle_body").html("body{ background-color:" + $("input[name=color_bg]").val() + "} .dzsportfolio, .dzsportfolio a{ color:" + $("input[name=color_main]").val() + "} .dzsportfolio .portitem:hover .the-title, .dzsportfolio .selector-con .categories .a-category.active { color:" + $("input[name=color_high]").val() + " }");
        });
        _t.trigger("change");
