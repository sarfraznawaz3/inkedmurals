...
class WC_Price_Calculator_Inventory {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_backordered_item_meta_name',      array( $this, 'get_backordered_item_meta_name' ), 10 );
	 ...
	 */
	public function get_backordered_item_meta_name( $backordered_text ) {

		$cart_contents = WC()->cart->get_cart();

		foreach ( $cart_contents as $key => $item ) {

			// we can just get the product for each and change its back order text
			// as this filter is only applied for backordered items
			$product = wc_get_product( $item['product_id'] );
